...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {  
  ...
  if (!is_initialized_) {
    ...
    if (meas_package.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      float vx = meas_package.raw_measurements_[2]* sin(meas_package.raw_measurements_[1]);
      float vy = meas_package.raw_measurements_[2]* sin(meas_package.raw_measurements_[1]);
      float v =  sqrt(vx*vx + vy*vy);      
      float yaw = acos(vx/v);
      float yawdd_ = 0;     

      x_ << px,py,v,yaw,yawdd_;
    }
  ...
  float dt = (meas_package.timestamp_ - previous_t)/1000000.0;
  Prediction(dt);
...
void UKF::Prediction(double dt) {
  ...
  // predict the mean and the covatiance //
